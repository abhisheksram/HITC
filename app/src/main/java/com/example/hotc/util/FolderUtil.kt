...
fun sortItemsByFolder(files: List<File>): Map<File, List<File>> {
    val resultMap = mutableMapOf<File, MutableList<File>>()
    for (file in files) {
        (!resultMap.containsKey(file.parentFile!!)).let {
            resultMap.put(
                file.parentFile!!,
                mutableListOf()
            )
...
fun getVideosFromFolder(context: Context, folder: String): List<File> {
